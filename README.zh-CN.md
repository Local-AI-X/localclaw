...
**LocalClaw** 是连接强大 AI 代理与普通用户的桥梁。它基于 [OpenClaw](https://github.com/OpenClaw) 构建，将命令行 AI 编排转化为易于访问、美观的桌面体验——无需终端操作。

无论您是自动化工作流、管理 AI 驱动的频道，还是调度智能任务，LocalClaw 都能提供有效利用 AI 代理所需的界面。

LocalClaw 预配置了最佳实践的模型提供商，原生支持 Windows 和多语言设置。当然，您也可以通过 **设置 → 高级 → 开发者模式** 微调高级配置。
...
**LocalClaw** 是一款可以部署本地大模型版本的 OpenClaw 客户端，专为普通用户设计，让每个人都能轻松使用本地大模型跑 OpenClaw，实现零成本使用，实现 AI 自由。
